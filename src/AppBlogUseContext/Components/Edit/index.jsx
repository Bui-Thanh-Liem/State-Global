...
function Edit(prop) {
    ...
    //
    const {dataEdit ,setDataEdit} = useContext(ContextEdit);
    ...
    // Component Unmounted state sẽ bị xóa khỏi bộ nhớ 
    useEffect(() => {
            ...
            <div className="edit-container">
                <form action="" onSubmit={handleSubmit}>
                    <div className="header">
                        <h3 style={{color: theme}} className="edit-title">Edit Profile</h3>
                        <button type="submit" onSubmit={handleSubmit} className="btn-save">Save</button>
                    </div>
                    <div className="input-container">
                        <InputGoup value={dataEdit.user} setValue={setUser} label="UserName"/>
                        <InputGoup value={dataEdit.age} setValue={setAge} label="Age"/>
                        <InputGoup value={dataEdit.about} setValue={setAbout} type="textarea" label="About"/>

                        <label className="title-ava">Avatar</label>
                        <div className="input-ava">
                            {urlAvatar.map((url, i) => <img 
                                                            onClick={(e) => setAvatar(e.target.src)} 
                                                            className={classNames("ava", {"active": url == avatar})}
                                                            key={i} 
                                                            src={url}
                                                        />
                                            )}
